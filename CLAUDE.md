...
| `waiting-list/decline.js` | Waitlister gives up a `Notified` hold → flips row to `Declined` (on-expired automation then releases the seat + notifies next) | POST | 10s |
...
- `Status` — single select: `Waiting`, `Notified`, `Converted`, `Expired`, `Removed`, `Declined` (`Declined` added June 2026 — waitlister actively gave up a Notified hold; see "Decline" below)
...
6. **On-expired re-notify** — Per-record Airtable automation triggered by Waiting List `Status` becoming **`Expired` OR `Declined`** (trigger widened June 2026 for the Decline feature — was `Expired` only). Runs `~/Documents/Vercel/airtable-scripts/SV-Ticketing-Base - waiting-list-on-expired.js`. Releases the linked Reservation (`Status=Released` → seat back to public pool if nobody else takes it), finds the next eligible `Waiting` row for the same Event, creates a fresh Reservation + token + 24h expiry, marks them `Notified`, and emits the same outputs as the cancellation script so the same email template body can be reused without re-mapping blue pills. The script reads only `waitingListId` and is indifferent to which status fired it, so a decline runs the identical cascade to an expiry. Why two automations (sweep + per-row) rather than one: Airtable's Send-email step is one email per automation run, so a burst of N expiries needs to fan out into N independent runs — the cron does the bulk mark; each per-row trigger does its own email.
...
**Decline — "I no longer want this ticket" (June 2026):** A `Notified` waitlister can actively give up their 24h hold so the seat passes to the next person immediately instead of waiting for the token to lapse. Two entry points, both on the Squarespace redemption page (`FRONTEND - WAITING LIST REDEMPTION 19th June '26.js`): an inline **"I no longer want this ticket"** link on the claim card (with a `confirm()`), and an email one-tap path `?token=...&action=decline` that routes to a dedicated **"Release your ticket?"** confirmation screen after the token validates (a confirm step — not an auto-decline on load — guards against accidental taps + email link-prefetchers). Both POST to `/api/waiting-list/decline`, which validates the token (same BOM-strip + `Redemption Token` lookup) and flips the row to `Status=Declined` — and **does nothing else**. The seat release + next-person notify is left entirely to the existing **on-expired automation** (step 6), whose trigger was widened to `Expired OR Declined`. One cascade path, no duplicated release/notify logic. The endpoint is idempotent (already-Declined/Expired → success; Converted → "already purchased"). **Prereqs (manual Airtable):** add the `Declined` option to `Status`; widen the on-expired trigger to "when record matches conditions → Status is any of Expired, Declined" (NOT "changes to Expired"). **Email decline button:** all three notify scripts (on-cancellation, on-expired, promote-from-hold) now also emit a `declineUrl` output (= `claimUrl + '&action=decline'`), emitted on the exact same notify path as `claimUrl` (every early return is upstream of it). Insert `declineUrl` as the ENTIRE href of the decline link/button — single blue pill, same BOM-avoidance rule as `claimUrl`. The emails inject raw inline-styled HTML `<a>` buttons, so the decline button can be a real red (`#ea3e28`) button matching the page's confirm button. To surface the new `declineUrl` token in Airtable's email-step variable picker you must run the script step once against a record that reaches the notify branch (a `Waiting` person must exist to be notified).
