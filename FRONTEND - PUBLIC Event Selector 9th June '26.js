...
/* ── Waiting List (Phase 1) ──────────────────────────────────────── */
/* The parent .ticket-sold-out has pointer-events:none — these re-enable
   interaction on the join button + form. */
...
.waitlist-form-wrapper.show {
    display: block;
}
.waitlist-form-wrapper label {
    display: block;
    margin-bottom: 10px;
    font-size: 13px;
}
.waitlist-form-wrapper input,
.waitlist-form-wrapper select {
    width: 100%;
    padding: 7px 9px;
    border: 1px solid #ccc;
    border-radius: 4px;
...
async function loadEvents() {
    try {
        ...
        // Only show public events on the public ticket page
        eventsData = eventsData.filter(function(event) { return event.eventType === 'Public Event'; });
...
document.getElementById('event-select').addEventListener('change', function() {
    ...
    if (selectedEventName) {
        ...
        ticketOptions.forEach(function(event) {
            ...
            // Waiting list — replaces qty controls with a join button for sold-out tickets
            if (event.ticketsRemaining <= 0) {
                buildWaitingListUI(event, controlDiv, ticketTypesList);
            }
...
// ── Waiting list (Phase 1) ────────────────────────────────────────────
function toggleWaitingListForm(eventId) {
    const form = document.getElementById('waitlist-form-' + eventId);
    if (form) form.classList.toggle('show');
}

function submitWaitingList(eventId) {
    const form = document.getElementById('waitlist-form-' + eventId);
    ...
    const payload = {
        eventId: eventId,
        firstName: form.querySelector('[name="wl-firstName"]').value.trim(),
        surname: form.querySelector('[name="wl-surname"]').value.trim(),
        email: form.querySelector('[name="wl-email"]').value.trim(),
        phone: form.querySelector('[name="wl-phone"]').value.trim(),
        quantityWanted: parseInt(form.querySelector('[name="wl-qty"]').value, 10) || 1
    };
    ...
    fetch('https://sv-ticket-scanner.vercel.app/api/waiting-list/join', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(function(res) {
        if (res.ok) {
            msgEl.textContent = res.data.message || 'You are on the waiting list.';
            msgEl.className = 'waitlist-message success';
            const fields = form.querySelectorAll('label, .waitlist-form-row, .waitlist-submit-btn');
            fields.forEach(function(el) { el.style.display = 'none'; });
        } else {
            msgEl.textContent = res.data.error || 'Could not add you to the waiting list.';
            msgEl.className = 'waitlist-message error';
...
function buildWaitingListUI(event, controlDiv, ticketTypesList) {
    // Replace the disabled qty controls with a "Join Waiting List" button
    controlDiv.innerHTML = '';
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'waitlist-trigger-btn';
    btn.textContent = 'Join Waiting List';
    btn.onclick = function() { toggleWaitingListForm(event.id); };
    controlDiv.appendChild(btn);

    // Append a hidden inline form below the row (expands on click)
